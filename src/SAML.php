...
class SAML
{
    public const META_KEY = 'pressbooks_saml_identity';

    public const SIGN_IN_PAGE = 'pb_saml_sign_in_page';

    public const USER_DATA = 'pb_saml_user_data';

    public const AUTH_DATA = 'pb_saml_auth_data';

    public const AUTH_N_REQUEST_ID = 'pb_saml_auth_n_request_id';

    // IMPORTANT: Do not rename to `pb_saml` - kept like this to be compatible with legacy integrations
    public const LOGIN_PREFIX = 'pb_shibboleth';

    // IMPORTANT: Do not rename to `/saml` - kept like this to be compatible with legacy integrations
    public const ENTITY_ID = '/shibboleth';

    public const SAML_MAP_FIELDS = [
    ...
    public function __construct(Admin $admin, Log $log = null)
    {
        ...
        if (! filter_var($options['idp_sso_login_url'], FILTER_VALIDATE_URL)) {
            add_action(
                'network_admin_notices',
                function () use ($configuration_error_message) {
